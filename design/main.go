...
func (s *sysadmin) administrate(system string) {
	fmt.Printf("Sys Admin = %+v, is on system: %+v\n", s.name, system)
}
...
func (p *programmer) develop(system string) {
	fmt.Printf("Programmer = %+v, is on system: %+v\n", p.name, system)
}
...
func main() {
	...
	admins.Enqueue(&sysadmin{"Neil Anderson"})

	// Enqueue two new programmers onto devs.
	devs.Enqueue(&programmer{"Tj Holowaychuk"})
	devs.Enqueue(&programmer{"Dave Cheney"})
